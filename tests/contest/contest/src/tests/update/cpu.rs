...
const CGROUP_NAME: &str = "cpu_test";

fn create_spec(resources: Option<LinuxResources>) -> Result<Spec> {
...
pub(crate) fn cpu_burst_test() -> TestResult {
    let spec = test_result!(create_spec(None));
...
pub(crate) fn set_cpu_period_without_quota_test() -> TestResult {
    ...
    let spec = test_result!(create_spec(Some(resources)));
...
pub(crate) fn set_cpu_period_without_quota_invalid_test() -> TestResult {
    ...
    test_outside_container(&spec, &|data| {
        if check_container_create(&data).is_ok() {
            return TestResult::Failed(anyhow!("expected container creation to fail with invalid cpu period"));
        }
        TestResult::Passed
    })
}
